#!/usr/bin/env python3
"""
Script simplificado para publicar cursos en GitHub Pages
SOLO PUBLICACIÓN WEB - Sin WhatsApp
"""
import os
import subprocess
import json
...
def commit_and_push_changes():
    ...
    commit_message = f"Actualizar cursos web - {timestamp}"
...
def create_web_workflow():
    """Crear workflow para automatizar la extracción web"""
    print("🤖 Creando workflow de automatización web...")
    
    # Crear directorio .github/workflows si no existe
    os.makedirs(".github/workflows", exist_ok=True)
    
    # Crear workflow de automatización web
    workflow_content = """name: Extract and Deploy Courses Web
  ...
  extract-courses:
    ...
    - name: Commit and push changes
      run: |
        ...
        git add .
        git diff --quiet && git diff --staged --quiet || git commit -m "Auto-update courses web $(date)"
        git push
...
"""
    ...
    with open(".github/workflows/auto-extract-web.yml", "w") as f:
        f.write(workflow_content)
    
    print("✅ Workflow de automatización web creado")
    return True
...
def main():
    """Función principal"""
    print("🚀 Publicador Web de GitHub Pages")
    print("=" * 40)
    print("📱 SOLO PUBLICACIÓN WEB - Sin WhatsApp")
    print("=" * 40)
    ...
    print("5. 📱 SOLO WEB - No se enviará a WhatsApp")
    
    if username and repo:
        print(f"\n🔗 Tu página estará disponible en:")
        print(f"   https://{username}.github.io/{repo}/")
    
    print("\n📱 NOTA: Este sistema SOLO publica en GitHub Pages")
    print("   No incluye funcionalidad de WhatsApp")
